...
為了確保抽獎活動的公正性與趣味性，本系統設計了多樣化的抽獎模式。本文件詳細說明各模式背後的程式邏輯與隨機算法，供技術人員與驗收單位參考。

## 🛡️ 核心亂數機制 (Core Randomness)

所有抽獎模式底層皆採用 JavaScript 標準的 `Math.random()` 函式生成亂數。
- 在現代瀏覽器 (V8 Engine 等) 中，`Math.random()` 使用 xorshift128+ 等高品質的偽隨機演算法，對於一般尾牙抽獎活動而言，其隨機性與分佈均勻度已非常足夠。
- **公平性保證**: 每次抽獎皆為獨立事件，系統**絕無內定**、**絕無黑箱**名單。所有「未中獎且未被取消資格」的參與者，中獎機率皆相等（除部門抽獎的加權設計外，詳見下文）。
