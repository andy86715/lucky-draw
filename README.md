...
這是一個網頁抽獎系統，採用溫馨可愛的粉色系（Sakura Pink）設計。系統支援多種抽獎模式，包含拉霸機、轉盤、部門抽獎、員編抽獎，並具備完整的後台管理功能。

## ✨ 功能特色

### 🎰 多樣化抽獎模式
1.  **拉霸機 (Slot Machine)**: 經典拉霸效果，支援最多抽出9位。
2.  **幸運轉盤 (Roulette)**: 物理模擬轉盤，視覺效果豐富，增加期待感。
3.  **員編抽獎 (Employee ID Draw)**: 遮罩解謎式抽獎，逐字揭曉員工編號，增加緊張感，支援最多抽出4位。
4.  **部門抽獎 (Department Draw)**: 兩階段式抽獎，先抽部門再抽該部門人員，搭配網格高亮動畫。
